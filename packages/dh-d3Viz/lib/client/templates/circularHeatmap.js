Template.circularheatmap.rendered = function() {
    ...
    var radialLabelsVal = Papa.parse(this.data.radialLabels).data,
        ...
        marginVal = {
            ...
            right: 0,
            bottom: 20,
            left: (width < 600)? 20 : width * 0.25
        };

    var chart = circularHeatChart()
        .segmentHeight(((height - (innerRadiusVal * 2) - marginVal.top - marginVal.bottom) / 2 )/ radialLabelsVal.length)
        .innerRadius(innerRadiusVal)
        .numSegments(segmentLabelsVal.length)        
        .radialLabels(radialLabelsVal)
        .segmentLabels(segmentLabelsVal)
        .margin(marginVal);

    d3.select('.circularHeatmapViz')
        .selectAll('svg')
        .data([circularHeatmapData])
        .enter()
        .append('svg')
        .attr("width", width)
        .attr("height", height)
    ...
    d3.selectAll(".circularHeatmapViz path").on('mouseover', function() {
    	var d = d3.select(this).data();
        d3.select("#info").text('Value: ' + d);
    });
