...
function commentMentionNotifications (comment) {
  ...
  if(Meteor.isServer && !comment.disableNotifications){
    ...
    // 4. Notify anyone who has been mentioned in the comment
    
    var userNames = comment.htmlBody.match(/(@)\w+/g);
    if (userNames !== null) {
    var mentionedIds = {}
    userNames.forEach(getUserIdFromDisplayName)

      var mentionIdsToNotify = _.difference(mentionedIds, userIdsNotified, [comment.userId]);
      Herald.createNotification(mentionedIds, {courier: 'newCommentSubscribed', data: notificationData});

      userIdsNotified = userIdsNotified.concat(mentionedIds);    
    }
    
    function getUserIdFromDisplayName(element, index, array) {
      element = element.replace("@","")
      mentionedIds.push(Users.getId(element));
