...
function postSubmitPush(post) {
    ...
    if (post.status === Posts.config.STATUS_PENDING && !!adminIds.length) {
        ...
        createPush('A new post has been submitted', pushData.post.title, notifiedUserIds);
    }
}
...
function postApprovedPush(post) {
    ...
    createPush('A new post has been approved', pushData.post.title, post.userId);
}
...
function commentSubmitPushs(comment) {
    ...
    if (Meteor.isServer && !comment.disablePushs) {
        ...
        if (Users.getSetting(postAuthor, "Pushs.comments", true) && comment.userId !== postAuthor._id) {
            ...
            createPush('New Comment', pushData.post.title, post.userId);
        }
        ...
        if (!!post.subscribers) {
            ...
            createPush('New Comment', pushData.post.title, subscriberIdsToNotify);
...
function createPush(titleVal, textVal, usersVal) {
    
    console.log(titleVal + " " + textVal + " " + usersVal);
